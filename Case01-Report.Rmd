...
$$R\_moment_1 \sim Fr + Re + St*(Fr, Re)$$
$$central_2 \sim Fr + Re + poly(St^{1/4}, 2) * (Fr, Re)$$
$$central_3 \sim Fr + Re + poly(St^{1/3}, 2) * (Fr, Re)$$
$$central_4 \sim Fr + Re + poly(\sqrt{St}, 2) * (Fr, Re)$$
...
Specifically, for each moment, we train candidate models to predict the moment with the general formula `~ poly(St^(1/root), degree)*(Fr, Re)` and vary the `degree` parameter from 1 to 3 and `root` from 1 to 6. Additionally, we may choose to take the log of the response. We then look at the estimated test errors and choose the parameters that give lower errors, with preference for less complexity if the error is similar. After selecting the features for each moment's model, we then fit the final models on the full training dataset using standard least-squares.
...
model1 <- train %>%
  tidyr::unite(interaction, Fr, Re, sep = ": ", remove = FALSE) %>%
  lm(central_1 ~ as.factor(Fr) + as.factor(Re) + poly(St, 1)*interaction, data = .)
summary(model1)
#plot(model1)
```
...
model2 <- train %>%
  mutate(St = St^(1/4)) %>%
  tidyr::unite(interaction, Fr, Re, sep = ": ", remove = FALSE) %>%
  lm(central_2 ~ as.factor(Fr) + as.factor(Re) + poly(St, 2)*interaction, data = .)
summary(model2)
#plot(model2)
```
...
model3 <- train %>%
  mutate(St = St^(1/3)) %>%
  tidyr::unite(interaction, Fr, Re, sep = ": ", remove = FALSE) %>%
  lm(central_3 ~ as.factor(Fr) + as.factor(Re) + poly(St, 2)*interaction, data = .)
summary(model3)
#plot(model3)
```
...
model4 <- train %>%
  mutate(St = sqrt(St)) %>%
  tidyr::unite(interaction, Fr, Re, sep = ": ", remove = FALSE) %>%
  lm(central_4 ~ as.factor(Fr) + as.factor(Re) + poly(St, 2)*interaction, data = .)
summary(model4)
#plot(model4)
```
